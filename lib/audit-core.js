...
async function captureCriterionEvidence(page, criterion, violations, viewportName, options) {
  const evidencePaths = buildEvidencePaths(options, viewportName, criterion.article);
  if (!evidencePaths) return [];
  ...
  for (const violation of violations) {
    for (const node of violation.nodes || []) {
      ...
      candidates.push({
        ruleId: violation.id,
        help: violation.help || violation.description || "Accessibility violation detected",
        helpUrl: violation.helpUrl || null,
        impact: node.impact || violation.impact || null,
        failureSummary: node.failureSummary || null,
        selector: selectorChain[0],
        selectorChain,
      });
    }
  }
  ...
  for (const node of deduped) {
    if (resolved.length >= MAX_EVIDENCE_NODES) break;
    try {
      ...
      // Skip selectors that cannot be resolved in Playwright.
    }
  }
  ...
  try {
    await addEvidenceOverlay(page, resolved.map((r) => r.box));
  ...
  return resolved.map((entry, index) => ({
    ...
    axeRuleId: entry.ruleId,
    impact: entry.impact,
    help: entry.help,
    helpUrl: entry.helpUrl,
    selector: entry.selector,
    selectorChain: entry.selectorChain,
    domPath: entry.domPath,
    elementHtml: entry.elementHtml,
    elementSource: entry.elementSource,
...
async function auditPageAtViewport(browser, url, viewport, options = {}) {
  ...
  try {
    ...
    for (const criterion of allCriteria) {
      ...
      const evidence = await captureCriterionEvidence(page, criterion, violations, vp.name, options);
      if (evidence.length > 0) evidenceByCriterion.set(criterion.article, evidence);
    }

    analysisResults = analysisResults.map((result) => {
      const evidence = evidenceByCriterion.get(result.criterion);
      if (!evidence || evidence.length === 0) return result;
          ...
          issues: [
            {
              ...
              message: "Visual evidence captured for this rule",
              evidence,
            },
          ],
        };
      ...
      const enrichedIssues = baseIssues.map((issue) => {
        const matched = evidence.filter((item) => item.help === issue.message);
        return {
          ...issue,
          evidence: matched.length > 0 ? matched : evidence,
        };
      });

      return { ...result, issues: enrichedIssues };
    });
    ...
    for (let i = 0; i < analysisResults.length; i++) {
      ...
      if (result.status !== COMPLIANCE_STATUS.NON_COMPLIANT && result.status !== COMPLIANCE_STATUS.NEEDS_REVIEW) continue;
      ...
      if (baseIssues.length === 0) {
        analysisResults[i] = {
          ...
          issues: [
            {
              type: "ai_candidate",
              message: "AI identified potential locations to verify",
              evidence: aiEvidence,
            },
          ],
        };
        ...
        analysisResults[i] = {
          ...result,
          issues: baseIssues.map((issue) => ({ ...issue, evidence: aiEvidence })),
        };
      }
    }
