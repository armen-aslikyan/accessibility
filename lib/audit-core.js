...
async function auditPageAtViewport(browser, url, viewport, options = {}) {
  ...
  const { concurrency = 2, llmModel = "mistral:7b-instruct-v0.3-q4_K_M", onProgress = null, onPhaseProgress = null } = options;
  ...
  try {
    ...
    for (let i = 0; i < analysisResults.length; i++) {
      ...
      console.log(`[audit-core] Capturing element detection evidence for ${criterion.article}`);
    ...
    for (let i = 0; i < analysisResults.length; i++) {
      ...
      console.log(`[audit-core] Capturing AI evidence for ${criterion.article}`);
      const aiEvidence = await Promise.race([
    ...
    // Post-process: resolve needs_review criteria using evidence occurrence counts and/or
    // the AI's preliminary assessment when no contradicting evidence exists.
    // Three cases handled in priority order:
    //   1. Explicit failed/passed occurrence counts → use counts
    //   2. All occurrences are in "review" state (AI-evidence) → use AI preliminary direction
    //   3. No evidence captured at all (e.g. DOCTYPE check) → use AI preliminary direction
    for (let i = 0; i < analysisResults.length; i++) {
      const result = analysisResults[i];
      if (result.status !== COMPLIANCE_STATUS.NEEDS_REVIEW) continue;
      ...
      for (const issue of Array.isArray(result.issues) ? result.issues : []) {
        totalFailed += issue.failedOccurrences || 0;
        totalPassed += issue.passedOccurrences || 0;
        totalReview += issue.needsReviewOccurrences || 0;
      }

      const totalEvidence = totalFailed + totalPassed + totalReview;

      if (totalFailed > 0 && totalPassed === 0) {
        // All explicit evidence points to failure
        analysisResults[i] = {
          ...result,
          status: COMPLIANCE_STATUS.NON_COMPLIANT,
          confidence: Math.max(result.confidence, 75 + Math.min(totalFailed * 2, 15)),
          reasoning: `${result.reasoning || ""} [Evidence-resolved: ${totalFailed} failing occurrence(s), 0 passing]`.trim(),
        };
      } else if (totalPassed > 0 && totalFailed === 0 && totalReview === 0) {
        // All explicit evidence points to passing
        analysisResults[i] = {
          ...result,
          status: COMPLIANCE_STATUS.COMPLIANT,
          confidence: Math.max(result.confidence, 75 + Math.min(totalPassed * 2, 15)),
          reasoning: `${result.reasoning || ""} [Evidence-resolved: ${totalPassed} passing occurrence(s), 0 failing]`.trim(),
        };
      } else if (result.preliminaryStatus === COMPLIANCE_STATUS.NOT_APPLICABLE) {
        // AI determined the criterion doesn't apply to this page
        analysisResults[i] = {
          ...result,
          status: COMPLIANCE_STATUS.NOT_APPLICABLE,
          confidence: Math.max(result.confidence, 95),
          reasoning: `${result.reasoning || ""} [Resolved via AI: not applicable]`.trim(),
        };
      } else if (result.preliminaryStatus && (totalEvidence === 0 || (totalReview > 0 && totalFailed === 0 && totalPassed === 0))) {
        // No explicit fail/pass counts but AI had a clear preliminary direction.
        // Covers: criteria with no capturable evidence (e.g. DOCTYPE) and AI-evidence
        // where all occurrences inherit the criterion's own needs_review status.
        analysisResults[i] = {
          ...result,
          status: result.preliminaryStatus,
          confidence: Math.max(result.confidence, 72),
          reasoning: `${result.reasoning || ""} [Resolved via AI preliminary assessment]`.trim(),
        };
      }
    }
