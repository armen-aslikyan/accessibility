...
export default function TemplateCard({ name, pageCount, representativeUrl, examplePaths, complianceRate, viewportResults }: Props) {
  const [expanded, setExpanded] = useState(false);

  const sortedViewports = [...viewportResults].sort((a, b) => VIEWPORT_ORDER.indexOf(a.viewport) - VIEWPORT_ORDER.indexOf(b.viewport));
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
      ...
      <div className="px-6 py-5">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              {hasViewportDiffs && <span className="bg-amber-100 text-amber-700 text-xs font-medium px-2 py-0.5 rounded-full">Viewport differences</span>}
            </div>
        ...
        {sortedViewports.length > 0 && (
          <div className="grid grid-cols-3 gap-2 mt-4">
            {sortedViewports.map((vr) => (
              <ComplianceBadge key={vr.viewport} rate={vr.complianceRate} label={vr.viewport.charAt(0).toUpperCase() + vr.viewport.slice(1)} />
            ))}
          </div>
        )}
          ...
          <div className="mt-4 space-y-1.5">
            {Object.entries(viewportIssues).map(([vp, articles]) => (
              <p key={vp} className="text-xs text-amber-700 bg-amber-50 rounded px-3 py-1.5">
                <span className="font-semibold capitalize">{vp}-only failures:</span> {articles.slice(0, 5).join(", ")}
        ...
        {examplePaths.length > 1 && (
          <div className="mt-3">
            <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-400 mb-1.5">Includes pages like</p>
        ...
        {/* Expand button */}
        {sortedViewports.length > 0 && (
          <button onClick={() => setExpanded((v) => !v)} className="mt-4 text-xs font-medium text-indigo-600 hover:text-indigo-800 transition">
