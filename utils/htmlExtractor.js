...
function extractForTheme(html, theme, options = {}) {
  const { maxElements = 50, maxChars = 4000, includeContext = true } = options;
...
function batchCheckApplicability(html, criteria) {
  ...
  for (const criterion of criteria) {
    ...
    if (specificSelector) {
      const specificCount = $(specificSelector).length;
      if (specificCount > 0) {
        results.set(article, { applicable: true, elementCount: specificCount, reason: null });
        continue;
      }
      // Specific selector returned 0 — confirm with theme-level selector before marking N/A.
      // This prevents false NOT_APPLICABLE when broader theme elements exist (e.g., images
      // exist on page but none match the narrow sub-selector for this sub-criterion).
      if (theme && !alwaysApplicableThemes.includes(theme)) {
        const themeSelector = THEME_SELECTORS[theme];
        if (themeSelector && $(themeSelector).length > 0) {
          // Theme has elements — let the LLM judge applicability for this specific criterion
          results.set(article, { applicable: true, elementCount: 0, reason: null });
          continue;
        }
      }
      // Both checks agree: no relevant elements
      results.set(article, {
        applicable: false,
        elementCount: 0,
