...
const StatementContent = ({ tFunc, data }) => {
  const {
    siteName,
    organizationName,
    complianceStatus,
  ...
  return (
    <>
      ...
      <section>
        ...
        <p>
          {siteName} {tFunc('statement.complianceText', { 
      ...
      <section>
        ...
        <p>{tFunc('statement.testResultsIntro', { auditor: auditorName, date: auditDate })}</p>
        <ul>
      ...
      <section>
        ...
        <ul>
          <li>
            {tFunc('statement.contactSendMessage')} <a href={contactFormUrl}>{tFunc('statement.contactFormLink')}</a>
          </li>
          <li>{tFunc('statement.contactEmail')} {data.contactEmail}</li>
        </ul>
      </section>
      ...
      <section>
        ...
        <ul>
          <li>
            {tFunc('statement.appealOption1')} <a href="https://formulaire.defenseurdesdroits.fr/">{tFunc('statement.appealOption1Link')}</a>
          </li>
          <li>{tFunc('statement.appealOption2')}</li>
          <li>
            {tFunc('statement.appealOption3')}<br />
...
const AccessibilityStatement = ({ data }) => {
  ...
  const frenchContentRef = useRef(null);
...
<head>
  ...
  <title>Déclaration d'accessibilité - ${siteName}</title>
  <style>
...
${content}
  ...
  const generateFrenchHTML = () => {
    return getStyledHTML(frenchContentRef.current.innerHTML);
  };

  const handleDownloadHTML = () => {
    const htmlContent = generateFrenchHTML();
    const blob = new Blob([htmlContent], { type: "text/html" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `declaration-accessibilite-${siteName.replace(/[^a-z0-9]/gi, '-').toLowerCase()}.html`;
  ...
  const handleDownloadPDF = () => {
    ...
    const options = {
      margin: 15,
      filename: `declaration-accessibilite-${siteName.replace(/[^a-z0-9]/gi, '-').toLowerCase()}.pdf`,
  ...
  return (
    <main className="accessibility-statement">
      ...
      <div>
        <StatementContent tFunc={t} data={data} />
      </div>

      {/* Hidden French content for exports */}
      <div ref={frenchContentRef} style={{ display: 'none' }}>
        <StatementContent tFunc={i18n.getFixedT('fr')} data={data} />
      </div>
    </main>
  );
};
